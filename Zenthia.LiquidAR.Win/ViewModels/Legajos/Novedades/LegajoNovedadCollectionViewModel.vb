...
Public Class LegajoNovedadCollectionViewModel
    Inherits CollectionViewModel(Of Zenthia.AccesoDatos.LegajosNovedades, Integer, IModeloDbContextUnitOfWork)

    Dim changedCustomProperties As List(Of Zenthia.AccesoDatos.LegajosNovedades) = New List(Of Zenthia.AccesoDatos.LegajosNovedades)()
    ...
    Public Overridable Property SelectedItems As IEnumerable(Of Zenthia.AccesoDatos.LegajosNovedades)

    Public Sub OnSelectedItemsChanged()
        Me.RaiseCanExecuteChanged(Sub(x) x.GenerateReciboAndSave())
    End Sub

    Public Property SelectedLegajos As List(Of Zenthia.AccesoDatos.Legajos)
    Public Property SelectedPeriodo As String
    Public Property SelectedTipoLiquidacion As Zenthia.AccesoDatos.TipoLiquidacion
    ...
    Public Overrides Sub Save(projectionEntity As AccesoDatos.LegajosNovedades)
        For Each item As Zenthia.AccesoDatos.LegajosNovedades In changedCustomProperties
            MyBase.Save(item)
        Next
    ...
    Public Sub GenerateReciboAndSave()
        ...
        For Each legajoNovedad As Zenthia.AccesoDatos.LegajosNovedades In SelectedItems
            Using db As Zenthia.AccesoDatos.Modelo = New Zenthia.AccesoDatos.Modelo()
                If db.Recibos.Where(Function(x) x.Legajos.Id = legajoNovedad.IdLegajo And x.Periodo = legajoNovedad.Periodo And x.IdTipoLiquidacion = legajoNovedad.IdTipoLoquidacion).Count = 0 Then
                    Dim _empresaactividad As Zenthia.AccesoDatos.EmpresasActividades = db.EmpresasActividades.Where(Function(x) x.IdEmpresa = legajoNovedad.Legajos.IdEmpresa And x.IdActividad = legajoNovedad.Legajos.idActividad).FirstOrDefault()

                    Dim recibo As New Zenthia.AccesoDatos.Recibos
                    ...
                    If id > 0 Then
                        ...
                        Dim formulas As IEnumerable(Of Zenthia.AccesoDatos.Formulas)
                        '**** Cargo los conceptos de las novedades  ****
                        For Each legajoformula As Zenthia.AccesoDatos.LegajosNovedadesDetalles In legajoNovedad.LegajosNovedadesDetalles
                            Dim formula As Zenthia.AccesoDatos.Formulas = db.Formulas.Where(Function(x) x.Id = legajoformula.IdFormula).FirstOrDefault
                            Dim recibodetalle As New Zenthia.AccesoDatos.RecibosDetalles
                        ...
                        For Each formula As Zenthia.AccesoDatos.Formulas In formulas
        ...
        Next
        SelectedItems = New Zenthia.AccesoDatos.LegajosNovedades(-1) {} 'Limpio la seleccion de la Grilla
    ...
    Public Sub GenerarNovedades()
        For Each legajo As Zenthia.AccesoDatos.Legajos In SelectedLegajos
            If Entities.Where(Function(x) x.Legajos.Id = legajo.Id And x.Periodo = SelectedPeriodo).Count = 0 Then
                Using db As Zenthia.AccesoDatos.Modelo = New Zenthia.AccesoDatos.Modelo()
                    Dim novedad As Zenthia.AccesoDatos.LegajosNovedades = New Zenthia.AccesoDatos.LegajosNovedades
    ...
    Public Sub NewWithParameters()
        'Dim initializer As Action(Of Zenthia.AccesoDatos.LegajosNovedades) = Function(x)
        '                                                                      x.IdLegajo = 1
        '                                                                      x.Periodo = "062021"
        '                                                                  End Function
        ...
        If Not Entities.FirstOrDefault() Is Nothing AndAlso Not Entities.FirstOrDefault().Legajos Is Nothing Then
            ...
            Dim recibo As Zenthia.AccesoDatos.Recibos = New Zenthia.AccesoDatos.Recibos
            clase.Entity = recibo
            clase.Recalcular()

            Dim service = Me.GetService(Of IDocumentManagerService)()
            service.ShowExistingEntityDocument(Of Zenthia.AccesoDatos.Legajos, Integer)(Me, Entities.FirstOrDefault().Legajos.Id)
...
'Dim recibo As Action(Of Zenthia.AccesoDatos.Recibos) = Function(x)
