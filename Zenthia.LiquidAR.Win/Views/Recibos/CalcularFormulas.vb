...
Public Class CalcularFormulas
    ...
    '------------------------------------------------------------------------------
    Private recibo As Zenthia.AccesoDatos.Recibos
    ...
    End Sub
    Public Sub New(recibo As Zenthia.AccesoDatos.Recibos)
        MyBase.New()
        Me.recibo = recibo
        ...
        If Len(sName) Then
            ...
            Else
                If (sName = "Remun") Then
                    ...
                    Dim remunerativo As Double = recibo.RecibosDetalles.Where(Function(w) w.Formulas.Conceptos.ColumnaRecibo = Zenthia.AccesoDatos.Entidades.enmColumnaRecivo.Remunerativo).Sum(Function(x) x.Importe)
                    sValue = remunerativo
                ElseIf (sName = "NoRemun") Then
                    'sValue = recibo.RecibosDetalles.Sum(Function(x) x.NoRemunerativo)
                    Dim noremunerativo As Double = recibo.RecibosDetalles.Where(Function(w) w.Formulas.Conceptos.ColumnaRecibo = Zenthia.AccesoDatos.Entidades.enmColumnaRecivo.NoRemunerativo).Sum(Function(x) x.Importe)
                ...
                ElseIf (sName = "Desc") Then
                    Dim desc As Double = recibo.RecibosDetalles.Where(Function(w) w.Formulas.Conceptos.ColumnaRecibo = Zenthia.AccesoDatos.Entidades.enmColumnaRecivo.Descuento).Sum(Function(x) x.Importe)
                    sValue = desc
                ...
                If sName.Substring(0, 1) = "C" AndAlso sName.Substring(sName.Length() - 1, 1) = "C" Then 'Caso de Cantidad en detalle
                    Dim _detalle As Zenthia.AccesoDatos.RecibosDetalles = recibo.RecibosDetalles.Where(Function(w) w.Formulas.Variable = sName.Substring(0, sName.Length - 1)).FirstOrDefault
