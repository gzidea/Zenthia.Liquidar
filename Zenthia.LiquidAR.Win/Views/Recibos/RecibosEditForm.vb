...
Public Class RecibosEditForm
    ...
    Private Sub InitBindings()
        ...
        'CondicionliquidacionComoEdit.Properties.AddEnum(Of Zenthia.AccesoDatos.Entidades.enmCondicionLiquidacion)()

        '//// Configuro la grilla de recibos detalles \\\\
        fluent.WithEvent(Of GridView, FocusedRowObjectChangedEventArgs)(gridView, "FocusedRowObjectChanged").SetBinding(Function(x) x.ReciboDetalles.SelectedEntity, Function(args) TryCast(args.Row, Zenthia.AccesoDatos.RecibosDetalles), Sub(gView, entity) gView.FocusedRowHandle = gView.FindRow(entity))
    ...
    Private Function SetSelection(ByVal gv As ColumnView, ByVal selItems As IEnumerable(Of Zenthia.AccesoDatos.RecibosDetalles)) As Boolean
        gv.BeginSelection()
        gv.ClearSelection()
        For Each it In (If(selItems, Enumerable.Empty(Of Zenthia.AccesoDatos.RecibosDetalles)()))
    ...
    Private Function GetSelection() As IEnumerable(Of Zenthia.AccesoDatos.RecibosDetalles)
        Return gridView.GetSelectedRows().[Select](Function(h) TryCast(gridView.GetRow(h), Zenthia.AccesoDatos.RecibosDetalles)).ToArray()
    End Function
