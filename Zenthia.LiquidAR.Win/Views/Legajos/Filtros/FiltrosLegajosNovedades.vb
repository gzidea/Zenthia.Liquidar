...
Public Class FiltrosLegajosNovedades
    ...
    Private Sub InitBindings()
        mvvmContext.ViewModelType = GetType(Zenthia.LiquidAR.Win.FiltrosLegajosNovedadesViewModel(Of Zenthia.AccesoDatos.LegajosNovedades, Integer, Zenthia.AccesoDatos.IModeloDbContextUnitOfWork))
        Dim fluent = mvvmContext.OfType(Of Zenthia.LiquidAR.Win.FiltrosLegajosNovedadesViewModel(Of Zenthia.AccesoDatos.LegajosNovedades, Integer, Zenthia.AccesoDatos.IModeloDbContextUnitOfWork))()
        ...
        fluent.WithEvent(Of ColumnView, FocusedRowObjectChangedEventArgs)(gridView, "FocusedRowObjectChanged").SetBinding(Function(x) x.SelectLegajo, Function(args) TryCast(args.Row, Zenthia.AccesoDatos.Legajos), Sub(gView, entity) gView.FocusedRowHandle = gView.FindRow(entity))
        ...
        'mvvmContextLegajos.AttachBehavior(Of GridCustomDrawRowIndicatorBehavior(Of Zenthia.AccesoDatos.LegajosNovedades))(gridView, Nothing, "LegajosNovedades", New Func(Of Zenthia.AccesoDatos.LegajosNovedades, Integer)(Function(d) d.Periodo = "052021")) 'imageCollection.Images(0))
    ...
    Private Function SetSelection(ByVal gv As ColumnView, ByVal selItems As IEnumerable(Of Zenthia.AccesoDatos.Legajos)) As Boolean
        gv.BeginSelection()
        gv.ClearSelection()
        For Each it In (If(selItems, Enumerable.Empty(Of Zenthia.AccesoDatos.Legajos)()))
    ...
    Private Function GetSelection() As IEnumerable(Of Zenthia.AccesoDatos.Legajos)
        Return gridView.GetSelectedRows().[Select](Function(h) TryCast(gridView.GetRow(h), Zenthia.AccesoDatos.Legajos)).ToArray()
    End Function
